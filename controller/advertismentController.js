const motorcycleService = require("../service/advertismentServices");

async function getAdvertismentById(req, res) {
  const { id } = req.body;
  console.log("Get advertisement by ID:", id);
  try {
    const advertisement = await motorcycleService.getAdvertismentById(id);
    if (!advertisement) {
...
const getBrands = async (req, res) => {
  ...
  try {
    const brandData = await motorcycleService.getBrands(page, size);
...
const getModels = async (req, res) => {
  ...
  try {
    ...
    const modelData = await motorcycleService.getModels(
      page,
      size,
      parsedFilter
    );
...
const getFloatingData = async (req, res) => {
  ...
  try {
    const floatingData = await motorcycleService.getFloatingData(
      brand,
      model,
      year
    );
...
const getMarketData = async (req, res) => {
  ...
  try {
    const parsedFilter = filter ? JSON.parse(filter) : {};
    const marketData = await motorcycleService.getMarketData(parsedFilter);
